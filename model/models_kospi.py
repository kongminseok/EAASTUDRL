...
from preprocessing.preprocessors_kospi import *
from config import config_kospi
from config.config_kospi import result
...
def train_ACKTR(env_train, model_name, timesteps=50000):
    ...
    model.save(f"{config_kospi.TRAINED_MODEL_DIR}/kospi_{model_name}")
...
def DRL_prediction(df,
    ...
    df_last_state.to_csv('results/{}/kospi_last_state_{}_{}.csv'.format(result, name, i), index=False)
    return last_state
...
def get_validation_sharpe(iteration):
    ...
    df_total_value = pd.read_csv('results/{}/kospi_account_value_validation_{}.csv'.format(result,iteration), index_col=0)
